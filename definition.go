...
func (d *Definition) AddArgument(value parameters.Value, spec string, desc string) {
	...
	if err != nil {
		panic(fmt.Errorf("definition: Error parsing argument specification: '%s'", err.Error()))
	}
	...
	if _, ok := d.arguments[arg.Name]; ok {
		panic(fmt.Errorf("definition: Cannot redeclare argument with name '%s'", arg.Name))
	}
...
func (d *Definition) AddOption(value parameters.Value, spec string, desc string) {
	...
	if err != nil {
		panic(fmt.Errorf("definition: Error parsing option specification: '%s'", err.Error()))
	}
	...
	for _, name := range opt.Names {
		if _, ok := d.options[name]; ok {
			panic(fmt.Errorf("definition: Cannot redeclare option with name '%s'", name))
		}
